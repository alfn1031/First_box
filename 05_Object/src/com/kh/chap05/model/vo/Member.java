...
public class Member {
	...
	// [ 생성자부 ]
	// 생성자(Constructor) : 객체 생성 시 초기화를 담당하는 특수한 구성요소
	/*
	 * 생성자의 두가지 역할
	 * -- 기술적인 관점 : 객체가 생성될 때 어떤 상태로 시작을 해야하는가
	 * -- 개념적인 간점 : 객체가 생성이 될거면 처음부터 유효한 상태여야 하지않아?
	 */
...
//	}
	...
	/*
	 * 1. 반드시 생성자의 이름을 클래스의 이름과 동일하게 작성해여 함(대/소문자)
	 * 2. 반환형이 존재해서는 안됨
	 * 3. 매개변수 생성자를 명시적으로 작성하면 기본생성자를 만들어주지 않음
	 * -> Legacy 기술들을 기본생성자가 필요한ㄴ 상황이 있음
