...
public class Run {
	...
	public static void main(String[] args) {
		 ...
		 * - 배열의 아주 쬐끔 불편한 점
		 * 
		 * 1. 배열을 사용하려고하면 무조건 크기 할당을 해야함 -> 한번 지저된 크기는 못바꿈
		 * 2. 배열 중간 위치에 새로운 요소를 추가하거나 삭제할 때 기존값을 밀거나 땡기는 코드를 써야함
		 * 
		 * - 컬랙션
		 * 1. 크기의 제약이 자유로움
		 * -> 처음 컬랙션을 사용할 때 크기를 지정하지 않아도 사용할 수 있음
		 * -> 만약 요소가 추가되면서 크기를 넘어서는 상황이 발생하면 알아서 확장됨
		 * 2. 중간에 요소를 추가하거나 삭제하는 경우 기존값을 밀거나 땡기는 코드가 이미 구현되어 있음
		 * -> 그때그때 필요한 메소드를 호출해서 사용하면 됨
		 * 
		 * - 중립
		 * - 배열의 경우 같은 타입의 자료형만 사용할 수 있음
		 * - 컬랙션의 경우 여러 타입의 데이터들을 사용할 수 있음 : 같은 타입의 데이터만 묶어서도 사용가능
		 * 
		 * 사용할 데이터의 개수가 명확히 정해져있고, 조회의 목적 -> 배열
		 * 데이터의 추가, 수정, 삭제가 빈번하게 일어난다 -> 컬렉션
		 * 
		 * 개념적인 컬랙션의 종류
		 * 
		 * - List 계열 : 다루려고하는 값(Value)만 저장 / 저장시 순서유지(Index), 중복값 허용
		 * 				-> LinkedList, ArrayList, Vector...
		 * 				-> ArrayList
		 * - Set 계열 : 다루려고하는 값(Value)만 저장 / 저장 시 순서유지 X, 중복값 허용 X
		 * 			  -> TreeSet, HashSet...
		 * 			  -> HashSet
		 * 
		 * - Map 계열 : 키(Key) - 값(Value) 세트로 저장 / 저장시 순서유지 X, 키 중복 X / 값 중복 O
		...
		// ArratList
		// ArratList 생성방법
		// ArratList 식별자 = new ArrayList(); -> 기본크기 10칸
		// ArratList 식별자 = new ArrayList(정수); -> 크기가 정수값으로 정해짐
		...
		// 자바에서 메소드 호출 시 전달할 수 있는 것은 오로지 값
		...
		// add메소드 오버로딩 엄청 잘돼있음!
		// add(int index, E e) : List의 index값에 전달할 요소를 추가
		plants.add(0, "식물 시작~");
		plants.add(3, new Plant("관엽", "홍콩야자"));
		System.out.println(plants);
		// 중간중간 위치에 요소를 추가했을 때 기존 요소를 한 칸씩 뒤로 밀어주는 코드가 구현되어있음
		
		// 2. 요소를 수정하는 메소드
		// set(int index, E e) : List의 index위치에 값을 전달할 요소(e)로 set해주는 메소드
		plants.set(0, "시이이이이작~");
		System.out.println(plants);
		...
		plants.remove(0);
		// plants.remove(5);
		// IndexOutputOfBoundException == insex의 값을 잘 고려해서 인자값을 전달해야 함
		...
		// index1부터 index2까지의 요소를 추출해서 새ㅗ운 리스트를 반환
		List sub = plants.subList(0, 2);
		System.out.println(sub);
		
		// 7. 리스트에다가 요소를 왕창 추가하기
		// addAll(Collection c) : 해당 리스트에 다른 컬랙션에 있는 요소들을 몽땅 추가해주는 메소드
		plants.addAll(sub);
