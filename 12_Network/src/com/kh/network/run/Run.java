...
public class Run {
	...
	public static void main(String[] args) {
		 ...
		 * 네트워크의 목적 : 데이터의 교환
		 * 
		 * 통신이 가능한 장치 == Host라고 통칭 
		 * 종단간의 실제 통신장치는 EndPoint라고 표현
		 * 
		 * IP(Internet Protocol) address / MAC(Media Access Controller) Address
		 * 
		 * - MAC Address : NIC(Network Interface Card)에 제조시 붙여지는 물리적인 주소
		 * 
		 * - IP Address : Internet상에서 각 Host들을 식별할수 있는 논리적인 주소
		 * 
		 * - Port Address : Host 내부에서 작동하는 Process를 식별할 때 사용하는 내부 번호 
		 * 
		 * MAC -> 이론상 절대 고정값 -> 실제 가상으로 올려서 사용함
		 * 
		 * -> IP주소와 Port번호는 항상 고정값이 아니라 변동이 가능한 유동적인 값
		 * 	  단, 값을 변경할 때 IP주소는 같은 네트워크 범위 안에서, Port번호는 컴퓨터 내부에서
		 * 	  중복이 발생되어서는 안됨!!! 충돌이 일어남!!!
		 *  
		 * DHCP(Dynamic Host Configuration Protocol)
		 * 
		 * - Server와 Client
		 * 
		 * Server : Client(사용자)에게 서비스를 제공해주는 물리적인 컴퓨터 또는 프로그햄
		 ...
		 * -> 클라이언트는 서버에 요청을 보내기 위해서는 서버의 IP주소와 Port번호를 알아야 함0 
		 */
		...
		try {
			...
			// domain name : 사용자가 서비스를 이용할 때 서버의 IP주소를 외우기 어려우니까
