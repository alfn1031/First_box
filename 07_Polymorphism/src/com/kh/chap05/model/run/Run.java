package com.kh.chap05.model.run;
	...
	public static void main(String[] args) {
		...
		for(int i = 0; i < soups.length; i++) {
			if(filter.test(soups[i])) {
				System.out.println(soups[i].getMainMenu());
			}
		}
		...
		// 9000원 이하인 김치찌개 찾는거 만들어주세요~~
		
		// 클래스를 따로 구현하지 않고 쓰는 자리에서 바로 구현할 수 있는 방법
		// : 익명클래스
		...
		// 1단계 : 익명클래스
		SoupFilter chiFilter = new SoupFilter() {
			@Override
			public boolean test(Soup s) {
				return s.getMainMenu().contains("치");
			}
		};
		// 2단계 : 인터페이스명, 에노테이션, 접근제한자, 반환형, 메소드명 싹 날리고
		// 		  (매개변수) -> {실행문}
