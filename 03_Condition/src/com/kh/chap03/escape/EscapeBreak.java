...
public class EscapeBreak {
	...
	public void method1() {
		...
		while(true) {
			System.out.print("글자수 체크(종료하고 싶으면 'exit'을 입력하세요) > ");
	...
	public void checkId() {
		...
		Scanner sc = new Scanner(System.in);
		// 사용자에게 아이디값을 입력받을 것인대 10글자가 넘으면 다시 입력하게 할 것
		//							  10글자가 넘지 않으면 다음 파트로 넘어갈 것
		...
		while(true) {
			...
			if(userId.length() <= 10) {
				...
				System.out.println("아이디는 10글자 이하만 사용가능합니다.");
				continue;
			}
