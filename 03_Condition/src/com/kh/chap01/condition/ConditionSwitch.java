...
public class ConditionSwitch {
	 ...
	 * 
	 * switch(정수 || 실수 || 문자 || 문자열) {
	 * case 값1 : 실행할코드a; // 종등비교대상 == 값1 true일 경우 실행할 코드 a를 수행
	 * break;
	 * case 값2 : 실행할코드b; // 동등비교대상 == 값2 true일 경우 실행할코드 b를 실행
	 * break;
	 * case 값3 : 실행할코드n; // 동등비교대상 == 값3 true일 경우 실행할코드 n를 실행
	 * break;
	 * drfault : 실행할코드;  // == else
	 * }
	 */
	
	// 조회하는 프로그램 -> 사람이름을 입력하면 정보ㅗ를 조회해주는 프로그램
	public void findByName() {
		...
		Scanner sc = new Scanner(System.in);
		System.out.print("검색할 성함을 입력해주세요. > ");
		...
		default : System.out.println("등록되지 않은 이름입니다.");
		}
		*/
		...
		return; // 현재 메소드의 영역을 아예 빠져나가는 구문 -> 나를 호출한 곳으로 돌아감
		} // break구문을 빠져나오는 부분
	...
	public void login() {
		...
		String id = "user01";
		String passward = "pass01";
		String name = "홍길동";
		
		// 1. 사용자에게 로그인 화면을 출력
		// 2. 사용자에게 아이디와 패스워드를 받음.
		// 3. 가입 시 입력했던 아이디와 비밀번호가 둘 다 정확히 일치한다면 로그인 성공화면을 출력
		// 4. 가입 시 입력했던 아이디 또는 비밀번호가 하나라도 일치하지 않으면 로그인 실패화면 출력
		...
		// 만약 사용자가 로그인 시 입력한 아이디값과 회원가입시 입력한 아이디값이 동일하고
		// 사용자가 로그인시 입력한 패스워드값과 회원이 입력한 패스워드 값이 동일하다면
		// 저장소바꾸고 / 유효성검증 / 비밀번호암호화
		if(userId.equals(id) && userPwd.equals(passward)) {
			System.out.println("로그인 성공" + name + "님 환영합니다");
		} else {
			System.out.println("아이디 또는 비밀번호가 일ㅊ치하지 않습니다.");
		}
		
		// 기본자료형 : int, short, long, char, boolean, float, double, byte
		// -> 기본자료형의 경우 동등비교연산자를 사용해서 비교연산을 수행함 (==, !=)
		
		// 참조자료형 : 나머지 전부
		// -> 동등비교연산자(==, !=)를 통해서 값을 비교하는것이 불가능
		// 문자열 값을 비교 : equals() -> true / false
		// 실제 변수에 값이 담기는 값이 '주소값'이기 때문
	}
